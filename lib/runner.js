...
app.on('ready', function() {
  ...
  parent.respondTo('runEventLoop', function(done) {
    function runBrowserEventLoop() {
      return win.webContents.executeJavaScript('window.nightmareRunEventLoop()')
    }
    runBrowserEventLoop().then(done)
  })
    ...
    function type() {
      ...
      runBrowserEventLoop().then(() => {
        ...
        runBrowserEventLoop().then(() => {
          ...
          win.webContents
            .executeJavaScript('window.nightmareWaitForNextFrame(1)')
            .then(type)
        })
      })
    }
