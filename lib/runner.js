...
app.on('ready', function() {
    ...
    if (win.webContents.getURL() == url) {
      ...
      var timer = setTimeout(function() {
        ...
        var error = domLoaded
          ? undefined
          : {
              message: 'navigation error (timed out)',
      ...
      function handleFailure(event, code, detail, failedUrl, isMainFrame) {
        if (isMainFrame) {
          cleanup({
            message: 'navigation error: ' + detail,
            code: code,
            details: detail,
            url: failedUrl || url
          })
      ...
      canLoadProtocol(protocol, function startLoad(canLoad) {
        ...
        cleanup({
          message: 'navigation error (unhandled protocol)',
          code: -1000,
  ...
  parent.respondTo('runEventLoop', function(done) {
    win.webContents
      .executeJavaScript('window.nightmareExpectEventLoopKey()')
      .then(done)
    win.webContents
      .executeJavaScript('window.nightmareRunEventLoop()')
      .then(() => {
        win.webContents.sendInputEvent({ type: 'keyUp', keyCode: 'F24' })
      })
  })

  parent.respondTo('waitForNextFrame', function(numFrames, done) {
    win.webContents
      .executeJavaScript(`window.nightmareWaitForNextFrame(${numFrames})`)
      .then(done)
  })
  ...
  parent.respondTo('type', function(value, done) {
    ...
    function type() {
      ...
      if (process.env.DEBUG == '*') console.log('Pressing ', ch)

      win.webContents
        .executeJavaScript('window.nightmareExpectKeyPress()')
        .then(() => {
          if (process.env.DEBUG == '*') console.log('Done pressing ', ch)
          type()
        })
